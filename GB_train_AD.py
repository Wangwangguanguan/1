...
def load_data(data_prefix, dataset_str, precalc):
  ...
  (num_data, train_adj, full_adj, feats, train_feats, test_feats, labels,
   train_data, val_data,
   test_data) = GB_utils.load_graphsage_data(data_prefix, dataset_str)
  ...
  train_mask = GB_utils.sample_mask(train_data, labels.shape[0])
  val_mask = GB_utils.sample_mask(val_data, labels.shape[0])
  test_mask = GB_utils.sample_mask(test_data, labels.shape[0])
  ...
  return (train_adj, full_adj, train_feats, test_feats, labels, y_train, y_val, y_test,
...
def evaluate(sess, model, val_features_batches, val_support_batches,
  ...
  for i in range(num_batches):
    ...
    if num_data_b == 0:
      ...
      feed_dict = GB_utils.construct_feed_dict(features_b, support_b, y_val_b,
  ...
  micro, macro = GB_utils.calc_f1(total_pred, total_lab, FLAGS.multilabel)
  return loss, acc, micro, macro

def main(unused_argv):
  """Main function for running experiments."""
  # Load data
  (train_adj, full_adj, train_feats, test_feats, labels, y_train, y_val, y_test,
  ...
  train_subgraph = nx.from_scipy_sparse_matrix(train_adj).subgraph(visible_data).copy()
  # tf.logging.info(
  #     "Number of nodes in the training subgraph: %d, number of edges: %d",
  #     train_subgraph.number_of_nodes(),
  #     train_subgraph.number_of_edges()
  # )

  GB_start_time = time.time()

  # The average degree is used to measure the quality of the granular-ball, with the square root of N initial granular-balls
  gb = GB_prop(init_GB_num=int(np.sqrt(len(train_subgraph))))

  # Check if there are any coarsening results of granular-balls
  dataset_name = FLAGS.dataset

  save_dir = f"./GB_graph_metis/{dataset_name}_coarsen"

  os.makedirs(save_dir, exist_ok=True)

  save_path = os.path.join(save_dir, f"{dataset_name}_coarse_graph.pickle")

  if os.path.exists(save_path):
      tf.logging.info(f"Find the saved coarsening result: {save_path}, and load it directly")
      with open(save_path, "rb") as f:
          GB_graph, clusters = pickle.load(f)
      # Convert clusters to parts format
      parts = [np.array(list(cluster)) for cluster in clusters]
      # Automatically set the number of clusters
      FLAGS.num_clusters = len(parts)
      tf.logging.info(f"Use granular-ball coarsening to generate {FLAGS.num_clusters} granular-balls")
  else:
      tf.logging.info("No coarsening result was found, performing granular-ball coarsening...")
      GB_graph, clusters = gb.get_GB_graph(train_subgraph)
      GB_end_time = time.time()
      # Record the coarsening time
      tf.logging.info('The coarsening takes time. %f seconds.', GB_end_time-GB_start_time)
      # Convert clusters to parts format
      parts = [np.array(list(cluster)) for cluster in clusters]
      # Automatically set the number of clusters
      FLAGS.num_clusters = len(parts)
      ...
      with open(save_path, "wb") as f:
          pickle.dump((GB_graph, clusters), f)
      tf.logging.info(f"The coarsening result has been saved to {save_path}")

  if FLAGS.bsize > 1:
    (features_batches, support_batches, y_train_batches,
     train_mask_batches) = GB_utils.preprocess_multicluster_with_parts(
         train_adj, parts, train_feats, y_train, train_mask,
         FLAGS.bsize, FLAGS.diag_lambda)
  else:
      (features_batches, support_batches, y_train_batches,
       train_mask_batches, batch_global_nodes) = GB_utils.preprocess_with_parts(
     train_adj, parts, train_feats, y_train,
     train_mask, FLAGS.diag_lambda)
  ...
  for epoch in range(FLAGS.epochs):
    ...
    if FLAGS.bsize > 1:
      ...
      for pid in range(len(features_batches)):
        ...
        feed_dict = GB_utils.construct_feed_dict(features_b, support_b, y_train_b,
