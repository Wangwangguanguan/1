...
def construct_feed_dict(features, support, labels, labels_mask, placeholders):
    """Construct a feed dictionary to feed TensorFlow"""
    
    # Handle different types of support input
    if isinstance(support, tuple):
        # support is already in a sparse matrix format and does not require processing
        pass
    elif hasattr(support, 'shape'):
        # support is a dense matrix and needs to be converted to a sparse format
        if support.shape == (1, 3):
            # Fix the matrix of shape (1,3)
            support = np.eye(3, dtype=np.float32)
        
        # Convert to a sparse matrix format
        from scipy import sparse
        support_sparse = sparse.coo_matrix(support)
        indices = np.vstack((support_sparse.row, support_sparse.col)).transpose()
        values = support_sparse.data
        shape = support_sparse.shape
        support = (indices, values, shape)
    
    # Calculate the number of non-zero features
    if hasattr(features, 'nnz'):
        num_features_nonzero = features.nnz
    else:
        num_features_nonzero = np.count_nonzero(features)
...
def preprocess_multicluster_with_parts(adj,
                                       parts,
                                       features,
                                       y_train,
                                       train_mask,
                                       block_size,
                                       diag_lambda=-1):
    """Generate batches for multiple clusters using externally provided parts (e.g., granular-ball clustering)."""
    features_batches = []
    support_batches = []
    y_train_batches = []
    train_mask_batches = []
    ...
    for _, st in enumerate(range(0, num_clusters, block_size)):
        ...
        if diag_lambda == -1:
            ...
            support_batches.append(sparse_to_tuple(normalize_adj_diag_enhance(support_now, diag_lambda)))
...
def preprocess_with_parts(adj,
                          parts,
                          features,
                          y_train,
                          train_mask,
                          diag_lambda=-1):
    """Preprocess data using externally provided parts (e.g., from granular-ball coarsening)."""
    features_batches = []
    support_batches = []
    y_train_batches = []
    train_mask_batches = []
    ...
    for pt in parts:
        ...
        if diag_lambda == -1:
            support_batches.append(sparse_to_tuple(normalize_adj(now_part)))
        else:
            support_batches.append(sparse_to_tuple(normalize_adj_diag_enhance(now_part, diag_lambda)))
        ...
        for newidx, idx in enumerate(pt):
            if train_mask[idx]:
                train_pt.append(newidx)
        train_mask_batches.append(sample_mask(train_pt, len(pt)))
        batch_global_nodes = [np.array(cluster) for cluster in parts]
    return features_batches, support_batches, y_train_batches, train_mask_batches, batch_global_nodes
